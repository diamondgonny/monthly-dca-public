...
from PortfolioRebalancingCore import get_portfolio_mkt_qty
from PortfolioRebalancingCore import get_buyable_cash_dollar_and_exchange_rate


def get_portfolio_from_google_sheet(worksheet: Worksheet, row_base_pointer: int):
    ...
    while True:
        ...
        row_pointer += 1
        print(ticker)
    buyable_cash_won = int(worksheet.cell(row_pointer, 8).value.lstrip('₩').replace(',', ''))
...
def put_mkt_qty_cash_to_google_sheet(worksheet: Worksheet, row_base_pointer: int, row_stack_pointer: int, buyable_cash_won: int,
                                     **portfolio_input):
    row_pointer = row_base_pointer
    print("Loading...")
    load_qty_from_api_activated = False
    for ticker, list in portfolio_input.items():
        if list[2] != 0:
            load_qty_from_api_activated = True
            break
    ...
    for index, (ticker, list) in enumerate(portfolio_input.items()):
        ...
        if load_qty_from_api_activated:
